 ...
 */
#define YIELD_ON_CHAR     if ( Serial.available() ) return; \
                          BLINK_LED
...
#define FLUSH_SERIAL_INPUT  do { Serial.read(); delay(100); } while ( Serial.available() > 0 );
...
void IncrementFrequencyTest ( void ) {
    ...
    float startHz = 1000, stopHz = 1e5, incHz = 10, sweepTimeSec = 30;
